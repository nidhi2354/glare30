          ...
          <ul className="hidden items-center gap-1 lg:flex">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="group relative rounded-full px-3.5 py-2 text-sm font-semibold text-navy-600 transition-colors hover:text-navy-800 xl:px-4"
                >
                  {link.label}
                  <span className="absolute inset-x-3.5 -bottom-0.5 h-0.5 origin-left scale-x-0 rounded-full bg-leaf-500 transition-transform duration-300 group-hover:scale-x-100 xl:inset-x-4" />
                </a>
              </li>
            ))}
          </ul>
        ...
        >
          <Container className="py-5">
              ...
              {navLinks.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    onClick={() => setOpen(false)}
                    className="flex items-center justify-between border-b border-navy-50 py-4 text-base font-semibold text-navy-700 transition-colors hover:text-leaf-600"
                  >
                    {link.label}
            ...
            <div className="mt-6 flex flex-col gap-3">
              <Button href="#contact" onClick={() => setOpen(false)} size="md" icon="arrowRight" className="w-full">
